...
class _DT {
  ...
  static const gradStart = Color(0xFF15803D);
  static const gradEnd = Color(0xFF22C55E);
...
class _StudentDashboardState extends State<StudentDashboard>
  ...
  void initState() {
    ...
    _focusedDay = now;
    final upcoming = _allEvents.where((e) => e.date.isAfter(now)).toList()
      ..sort((a, b) => a.date.compareTo(b.date));
    _selectedDay = upcoming.isNotEmpty ? upcoming.first.date : now;
  ...
  List<Event> _eventsForDay(DateTime day) {
    final now = DateTime.now();
    return _allEvents
        .where((e) => isSameDay(e.date, day) && e.date.isAfter(now))
        .toList();
  }
              ...
              todayBuilder: (ctx, day, _) => LayoutBuilder(builder: (ctx, c) {
                ...
                return Center(
                  ...
                  decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle,
                      border: Border.all(
                          color: const Color(0xFF16A34A), width: 1.8)),
                  alignment: Alignment.center,
                  child: Text('${day.day}',
                      style: const TextStyle(
        ...
        builder: (_) => Scaffold(
          ...
          body: Center(
            child: InteractiveViewer(
              minScale: 0.8,
              maxScale: 4.0,
              child: Image(image: _imageProvider(url), fit: BoxFit.contain),
            ),
          ),
        ),
      ),
              ...
              children: [
                ...
                Align(
                  ...
                  child: TextButton.icon(
                    onPressed: () {
                      showEventComments(context, eventId: widget.event.id, eventTitle: widget.event.title);
                    },

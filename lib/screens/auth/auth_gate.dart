...
import '../Adviser_Role/adviser_dashboard_screen.dart';
import 'unified_login_page.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'get_started_screen.dart';
import '../guest/public_events_screen.dart';
...
class _AuthGateState extends State<AuthGate> {
  // ── Palette ───────────────────────────────────────────────────────────────
  static const Color _kBg = Color(0xFFF1F8F4);
  static const Color _kTitle = Color(0xFF17352A);
  static const Color _kSubtitle = Color(0xFF6B8178);
  static const Color _kBorder = Color(0xFFDCEBE4);
  static const Color _kLoader = Color(0xFF128058);
...
// After
    if (session == null) {
      ...
      // Public events are intentionally visible before authentication so a
      // student can discover current organization activity without logging in.
      _goTo(const PublicEventsScreen());
      return;
    }
    ...
    try {
      ...
      if (role == 'admin') {
        ...
        AppState.instance.setRole(UserRole.adviser);
        AppState.instance.setCurrentOfficerOrgId(assignedOrgId);
        _goTo(AdviserDashboard(orgId: assignedOrgId, orgName: orgs[orgIndex].name));

...
class OfficerMembersScreen extends StatelessWidget {
  ...
  static Color _posColor(String pos) {
    switch (pos.toLowerCase()) {
      case 'president':       return const Color(0xFF4F46E5);
      case 'vice president':  return const Color(0xFF06B6D4);
      case 'secretary':       return const Color(0xFF0D9488);
      case 'officer':         return const Color(0xFF22C55E);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          ...
          SliverAppBar(
            ...
            title: const Text('Members', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 17, letterSpacing: 0.3)),
            centerTitle: true,
            flexibleSpace: FlexibleSpaceBar(background: _GradientHero(
      ...
      floatingActionButton: _GradFab(
        icon: Icons.person_add_rounded, label: 'Add Member',
        onTap: () => _dialog(context),
      ),
    );
  }

  // ── Member dialog (all original logic) ──────────────────────────────────
    ...
    showDialog(
      ...
      builder: (_) => StatefulBuilder(builder: (ctx, ss) => Dialog(
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(
          ...
          children: [
            _DialogHeader(existing == null ? Icons.person_add_rounded : Icons.edit_rounded,
                existing == null ? 'Add Member' : 'Edit Member'),
            const SizedBox(height: 20),
            _Field(ctrl: nameCtrl, label: 'Full Name',     hint: 'Enter member name',       icon: Icons.person_rounded),
            ...
            DropdownButtonFormField<String>(
              ...
              items: ['Member','Officer','President','Vice President','Secretary']
                  .map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(),
              onChanged: (v) => ss(() => position = v ?? 'Member'),
            ),
            const SizedBox(height: 14),
            _Field(ctrl: orgCtrl, label: 'Organization', hint: 'Enter organization name', icon: Icons.groups_rounded),
            ...
            Row(children: [
              ...
              Expanded(child: _SaveBtn(onTap: () {
                ...
                if (existing == null) {
                  ...
                  _snack(context, 'Added $name', _ok);
                } else {
                  AppState.instance.updateMember(existing.copyWith(name: name, position: position, orgName: org));
                  _snack(context, 'Updated $name', _ok);
                }
                Navigator.pop(ctx);
              })),
            ]),
...
class _MemberCard extends StatelessWidget {
  final Member member; final int index;
  final Color accentColor;
  final VoidCallback onEdit, onDelete;
  const _MemberCard({required this.member, required this.index, required this.accentColor, required this.onEdit, required this.onDelete});
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      duration: Duration(milliseconds: 240 + index * 50), curve: Curves.easeOut,
      builder: (_, v, child) => Opacity(opacity: v, child: Transform.translate(offset: Offset(0, 12*(1-v)), child: child)),
      child: Container(
        margin: const EdgeInsets.only(bottom: 10),
        decoration: BoxDecoration(
          color: Colors.white, borderRadius: BorderRadius.circular(16),
          boxShadow: [BoxShadow(color: const Color(0xFF4F46E5).withOpacity(0.06), blurRadius: 20, offset: const Offset(0, 6))],
        ),
        child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), child: Row(children: [
          ...
          Container(width: 46, height: 46,
            ...
            child: Center(child: Text(initial, style: TextStyle(color: accentColor, fontSize: 18, fontWeight: FontWeight.w800))),
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(member.orgName, style: const TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
            const SizedBox(height: 5),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(color: accentColor.withOpacity(0.10), borderRadius: BorderRadius.circular(20)),
              child: Text(member.position, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: accentColor)),
            ),
          ])),
...
class _GradientHero extends StatelessWidget {
  ...
  const _GradientHero({required this.icon, required this.title, required this.subtitle});
      ...
      SafeArea(child: Padding(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(subtitle, style: TextStyle(color: Colors.white.withOpacity(0.72), fontSize: 12.5)),
          ]),
        ]),
      )),
    ]),
  ...
  Widget build(BuildContext context) => Tooltip(message: tip,
    child: Material(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(10),
      child: InkWell(borderRadius: BorderRadius.circular(10), onTap: onTap,
        child: Padding(padding: const EdgeInsets.all(8), child: Icon(icon, color: color, size: 17)))));
}

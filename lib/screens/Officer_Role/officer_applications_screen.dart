...
class OfficerApplicationsScreen extends StatelessWidget {
  ...
  static const _background = Color(0xFFF0FDF4);
  ...
  static _SC _sc(ApplicationStatus s) {
    switch (s) {
      ...
      case ApplicationStatus.interviewed:
        return const _SC('Interviewee', Color(0xFFDCFCE7), Color(0xFF3730A3),
            Color(0xFFC7D2FE));
      ...
      case ApplicationStatus.interview_scheduled:
        return const _SC('Scheduled', Color(0xFFDCFCE7), Color(0xFF3730A3),
            Color(0xFFC7D2FE));
...
// Filtered view
  ...
  void _finalApprove(BuildContext context, Application a) async {
    ...
    try {
      final approved = await AppState.instance.approveApplication(a.id);
      if (!approved) {
      ...
      if (context.mounted) {
        _snack(
          context,
          '${a.name} approved and added as a member!',
  ...
  Future<void> _decline(BuildContext context, Application a) async {
    final controller = TextEditingController();
    final reason = await showDialog<String>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Decline application'),
        content: TextField(controller: controller, maxLines: 5, decoration: const InputDecoration(labelText: 'Reason for not qualifying', hintText: 'Explain what the student can improve or why they did not qualify.')),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          FilledButton(onPressed: () { if (controller.text.trim().isNotEmpty) Navigator.pop(context, controller.text.trim()); }, child: const Text('Send Reason')),
        ],
      ),
    );
    controller.dispose();
    if (reason == null) return;
    await AppState.instance.declineApplication(a.id, reason);
    if (context.mounted) _snack(context, 'Application declined and reason sent to the student.', Colors.redAccent);
  }
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _infoCard([
                      ...
                      if (a.status == ApplicationStatus.declined && a.declineReason.isNotEmpty) _detailRow('Reason', a.declineReason),
                    ...
                    if (a.attachments.isEmpty)
                      _infoCard([
                        const Padding(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.attach_file_rounded, color: Color(0xFF94A3B8)),
                              SizedBox(width: 10),
                              Expanded(child: Text('No attachments were submitted with this application.')),
                            ],
                          ),
                        ),
                      ])
                        ...
                        child: Column(
                          ...
                          children: a.attachments.asMap().entries.map((entry) {
                            ...
                            if (imgExts.contains(ext)) {
                              ...
                              // Every attachment remains visible even when its
                              // extension is unknown. Tapping it opens the file.
                              return Padding(
                                ...
                                child: ListTile(
                                  ...
                                  leading: const Icon(Icons.insert_drive_file_rounded, color: _primary),
                                  title: Text(
                                  ...
                                  subtitle: const Text('Tap to open attachment'),
  ...
  Widget build(BuildContext context) => Material(
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(icon, color: const Color(0xFF16A34A), size: 14),
              const SizedBox(width: 6),
              Flexible(
                  child: Text(label,
                      style: const TextStyle(
